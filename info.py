...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1859932464').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002308324197').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(ch) else ch for ch in environ.get('AUTH_USERS', '').split()]
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1001827067676') # support group id ( make sure bot is admin )
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1001974447080') # request channel id ( make sure bot is admin )
